...
class MusicDetail extends Component{
    ...
    return(
            <View style={{ backgrounColor:'blue', flexDirection:'column'}}>
                <View style={{backgroundColor:'skyblue',width:(Dimensions.get('window').width/2)}}>
                <Text>曲目名称： {music.title} </Text>
                 <Text> 评分，评价：{music.score},{music.pjs} </Text>
                 <Text>{music.info1}</Text>
                 </View>
                 <Image source={{uri:music.pic}} style={{width:(Dimensions.get('window').width/2.2),height:50}} />
            </View>
        );
    }
}
